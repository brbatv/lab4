...
To access to one of the previews statistics it may be used *$* as it follows:
...
The package provides also three other functions which return some of the statistics showed before: 
...
In `lab4ab` package there are two graphs: one representing the residuals vs the fitted values, the second one a scaled version of the first one, the residuals have been standardized by the formula: $$\sqrt{ \frac{|mean(res)-res|}{\sqrt{{Var(res)}}}}$$ .
The red line represents the trend of the observations in the graphs. The smooth has been realized with a local polynomial regression fitting with a span (parameter which controls the degree of smoothing) of 1.5. It looks very close to the one obtained by `plot(lm())`. The function `plot()` plots using *ggplot2* package that has to be installed before using `lab4ab`. 
...
Last function in the package is `summary()`. It's aim is to return a similar printout as printed for `lm()`. The statistics of this output are: 
...
## prendi come esempio per i plot:

The figure sizes have been customised so that you can easily put two images side-by-side. 

```{r, fig.show='hold',fig.cal="Residuals vs Fitted values"}
plot(1:10)
plot(10:1)
```
